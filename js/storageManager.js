...
function loadGlobalStyles() {
  try {
    ...
    if (saved) {
      ...
      if (elements.shadowOpacity) elements.shadowOpacity.value = styles.shadowOpacity;
      if (elements.letterSpacing) elements.letterSpacing.value = styles.letterSpacing;
...
function loadFileData() {
  try {
    ...
    if (saved) {
      ...
      if (AppState.currentFileType && Object.keys(AppState.conversationPairsMap).length > 0) {
        ...
        console.log('[loadFileData] AppState.youCount 복원:', AppState.youCount);

        // YOU2, YOU3 등 추가 드롭다운 생성 (youCount > 1인 경우)
        if (fileData.youCount > 1) {
          console.log('[loadFileData] YOU2+ 드롭다운 생성 시작, youCount:', fileData.youCount);
          ...
          for (let i = 2; i <= fileData.youCount; i++) {
            ...
            // 저장된 색상 또는 기본 색상 사용
            const savedColors = fileData.autoYouColors && fileData.autoYouColors[`youCharacter${i}`];
            ...
            newDiv.innerHTML = `<label style="margin: 0;">
              YOU${i}:
              <select id="youCharacter${i}">
                <option value="">선택하세요</option>
                ${allCharacters.map(char => `<option value="${char}">${char}</option>`).join('')}
              </select>
            </label>
            <label style="margin: 0; font-size: 12px;">배경<input type="color" id="youBg_youCharacter${i}" value="${defaultBg}" style="width: 40px; height: 25px;" /></label>
            <label style="margin: 0; font-size: 12px;">글자<input type="color" id="youColor_youCharacter${i}" value="${defaultColor}" style="width: 40px; height: 25px;" /></label>
            <button style="margin-top: 22px; margin-left: 5px; padding: 2px 9px 4px 9px;" onclick="removeYouCharacter(${i})">-</button>`;
        ...
        if (fileData.autoYouColors) {
          console.log('[loadFileData] YOU 색상 복원 시작:', fileData.autoYouColors);
          Object.keys(fileData.autoYouColors).forEach(youId => {
            const bgInput = document.getElementById(`youBg_${youId}`);
            const colorInput = document.getElementById(`youColor_${youId}`);
            if (bgInput) {
              bgInput.value = fileData.autoYouColors[youId].bg;
              console.log(`[loadFileData] ${youId} 배경색 복원:`, fileData.autoYouColors[youId].bg);
            } else {
              console.warn(`[loadFileData] ${youId} 배경 input을 찾을 수 없음!`);
            }
            if (colorInput) {
              colorInput.value = fileData.autoYouColors[youId].color;
              console.log(`[loadFileData] ${youId} 글자색 복원:`, fileData.autoYouColors[youId].color);
            } else {
              console.warn(`[loadFileData] ${youId} 글자 input을 찾을 수 없음!`);
            }
          });
        }
        ...
        requestAnimationFrame(() => {
          console.log('[loadFileData] updateAfterStyles 호출 (2차 - requestAnimationFrame)');
          if (typeof updateAfterStyles === 'function') {
            updateAfterStyles();
          }
          // 추가 안전장치: 한 번 더 확인
          setTimeout(() => {
            console.log('[loadFileData] updateAfterStyles 호출 (3차 - setTimeout 20ms)');
            if (typeof updateAfterStyles === 'function') {
              updateAfterStyles();
            }
          }, 20);
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (resetBtn) {
    resetBtn.addEventListener('click', () => {
      if (confirm('모든 데이터를 초기화하시겠습니까? 페이지가 새로고침됩니다.')) {
        clearAllData();
      }
    });
  }
