...
function updateAutoGeneratedColors() {
  ...
  containers.forEach((container) => {
    ...
    if (!bubble || bubble.dataset.source !== "auto") return;

    // ME 말풍선 업데이트 (me-auto 클래스)
    if (bubble.classList.contains("speech-bubble-me-auto")) {
      bubble.style.backgroundColor = meColors.bg;
      bubble.style.color = meColors.color;
    }
    ...
    for (const youId of youIds) {
      const suffix = getYouClassSuffix(youId);
      if (bubble.classList.contains(`speech-bubble-${suffix}`)) {
        const youColors = getYouColors(youId);

        bubble.style.backgroundColor = youColors.bg;
        bubble.style.color = youColors.color;
        break;
      }
    }
  });
